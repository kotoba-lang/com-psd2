(ns psd2.main
  "Kotodama WASM entrypoint for the PSD2 clean-room actor (L4) — Clojure port.
  ...
  facts (`psd2.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"psd2.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "psd2-compat" :tier tier :entities entities} 200])
